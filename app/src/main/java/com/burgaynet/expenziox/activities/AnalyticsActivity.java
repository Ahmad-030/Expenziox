...
import com.burgaynet.expenziox.utils.BarChartView;
import com.burgaynet.expenziox.utils.DataManager;
import com.burgaynet.expenziox.utils.PieChartView;
...
public class AnalyticsActivity extends AppCompatActivity {
    ...
    private void loadCharts() {
        ...
        for (int i = 5; i >= 0; i--) {
            ...
            bars.add(new BarChartView.Bar(label, (float) monthTotal,
                    isCurrent ? 0xFF00D4AA : 0xFF1565C0));
        }
    ...
    private void addLegendRow(String category, double amount, int color) {
        ...
        LinearLayout.LayoutParams catLp = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
    ...
    private void setupBottomNav() {
        ...
        findViewById(R.id.navBudget).setOnClickListener(v -> { startActivity(new android.content.Intent(this, BudgetActivity.class)); overridePendingTransition(R.anim.fade_in, R.anim.fade_out); });
    ...
    private void highlightNav(int idx) {
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            TextView icon = (TextView) nav.getChildAt(0);
            TextView label = (TextView) nav.getChildAt(1);
            boolean sel = i == idx;
            label.setTextColor(sel ? 0xFF00D4AA : 0xFF4A5568);
            icon.setAlpha(sel ? 1f : 0.5f);
        }
    }
